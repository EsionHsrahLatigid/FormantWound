...
int main(int argc, char** argv)
{
    if (argc != 3)
    {
        std::cerr << "usage: FormantWoundPluginLoadTest <vst3-bundle> <expected-name>\n";
        return 2;
    }
    ...
    constexpr const char* parameterNames[] {
        "LPC Resolution", "Excitation Corruption", "Formant Warp", "Freeze Hold",
        "Reseed", "Damage", "Feedback", "Mix", "Output"
    };
    for (const auto* name : parameterNames)
        if (findParameter(*instance, name) == nullptr)
            return failure(9, "hosted parameter set is incomplete");

    findParameter(*instance, "LPC Resolution")->setValueNotifyingHost(1.0f);
    findParameter(*instance, "Excitation Corruption")->setValueNotifyingHost(1.0f);
    findParameter(*instance, "Formant Warp")->setValueNotifyingHost(1.0f);
    findParameter(*instance, "Reseed")->setValueNotifyingHost(0.73f);
    findParameter(*instance, "Damage")->setValueNotifyingHost(1.0f);
    ...
    findParameter(*instance, "Damage")->setValueNotifyingHost(0.0f);
    instance->setStateInformation(state.getData(), static_cast<int>(state.getSize()));
    if (state.isEmpty() || findParameter(*instance, "Damage")->getValue() < 0.99f)
    ...
    if (rms <= 0.02 || peak > 0.981f || std::abs(dc) >= rms * 0.8
